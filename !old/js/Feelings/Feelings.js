...
function determineFeelings(Person) {
	let feelingsReviewed = [];
	let combinationFeelingsReviewed = [];
	...
	Person.MentalHealth.Feelings.forEach(feeling => {
		...
		const uniqueFeelingName = `${feeling.Combination[0]}-${feeling.Combination[1]}`;
		const foundFeeling = feelingsReviewed.find(item => item === uniqueFeelingName);
		if(!foundFeeling) {
			...
			feeling.Combination.forEach(combination => {
				const foundCombinationFeeling = combinationFeelingsReviewed.find(item => item === combination);
				
				if(!foundCombinationFeeling) {
					...
					combinationFeelingsReviewed.push(combination);
				}
				
			});

			feelingsReviewed.push(uniqueFeelingName);
		}
