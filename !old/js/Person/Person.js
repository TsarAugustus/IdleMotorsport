...
export function Person(randomAttributesBoolean, People) {
	...
	const randomTrackType = trackTypes[Math.floor(Math.random() * trackTypes.length)];
	NewPerson.Preferences.TrackType.Style = randomTrackType.style;
	NewPerson.Preferences.TrackType.Type = randomTrackType.type[Math.floor(Math.random() * randomTrackType.type.length)].name;
	...
	if(randomAttributesBoolean === true) {
		NewPerson.Money = randomNumber(0, 1000000);
