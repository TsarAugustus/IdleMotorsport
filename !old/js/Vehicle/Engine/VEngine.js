export function VEngine(configuration, thisManufacturer) {
	...
	//mostly to find the compression ratio
	const cylinderHeight = strokeLength + randomNumber(1, 5);
	const displacementVolume = Math.PI * boreDiameter * cylinderHeight;
	const sweptVolumeCC = Math.pow((boreDiameter / 2), 2) * Math.PI * 8.09;
	
	const cylinderVolume = Math.PI * boreDiameter * cylinderHeight;
	//Dry, because Fuel can improve the compression ratio
	const compressionRatioDry = (displacementVolume + sweptVolumeCC) / sweptVolumeCC;
